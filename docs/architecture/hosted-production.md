...
| Step | Work | Status |
|------|------|--------|
| H1 | Runtime adapter interfaces + local implementations | **Done** |
| H2 | Postgres schema + JobStore/RunCatalogStore | **Done** |
| H3 | Supabase Storage artifact read path via `/api/data` | **Done** |
| H4 | Hosted job API gating (beta, caps, executor check) | **Done** |
| H5 | Trigger.dev worker + Python `worker run-job` | **Done** |
| H6 | Hosted UI (beta code, job polling) | **Done** |
| H7 | Deploy docs and env runbook | **Done** |
