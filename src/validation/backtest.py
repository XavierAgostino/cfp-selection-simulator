...
class ColleyMatrix:
    ...
    def __init__(self, games_df: pd.DataFrame):
        ...
        self.teams = sorted(set(games_df['home_team'].unique()) | set(games_df['away_team'].unique()))
...
class EloRatings:
    ...
    def __init__(self, k_factor: int = 85, hfa_points: float = 3.75, mov_scale: int = 17, mov_cap: int = 28):
        self.k = k_factor
    ...
    def update_game(self, home_team: str, away_team: str, home_score: int, 
                   away_score: int, is_neutral: bool):
    ...
    def process_season(self, games_df: pd.DataFrame):
        ...
        teams = sorted(set(games_sorted['home_team'].unique()) | set(games_sorted['away_team'].unique()))
...
def calculate_composite_rankings(games_df: pd.DataFrame) -> pd.DataFrame:
    ...
    resume_scores = {teams[i]: float(0.6 * colley_norm[i] + 0.4 * win_pct_norm[i]) 
                     for i in range(len(teams))}
    predictive_scores = {teams[i]: float(0.5 * massey_norm[i] + 0.5 * elo_norm[i]) 
                        for i in range(len(teams))}
    ...
    def get_opponent_records(team: str):
        ...
        for _, game in team_games.iterrows():
            ...
            for _, g in opp_games.iterrows():
                if (g['home_team'] == team and g['away_team'] == opp) or (g['home_team'] == opp and g['away_team'] == team):
    ...
    for team in teams:
        ...
        sos_scores[team] = calculate_sos(opp_records, opp_opp_records, include_oor=True, oor_weight=0.33)
...
def calculate_spearman_correlation(
    ...
    """
    common_teams = [team for team in cfp_rankings 
                   if team in simulator_rankings['team'].values]
