...
function ResumeHeader({ resume }: { resume: TeamResume }) {
  return (
...
function WhyInConcerns({ resume }: { resume: TeamResume }) {
  if (resume.why_in.length === 0 && resume.concerns.length === 0) return null;
  return (
    <div className="flex flex-col gap-4">
      {resume.why_in.length > 0 ? (
        <div>
          <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-foreground">
            Why they&apos;re in
          </h3>
          <ul className="flex flex-col gap-1.5">
            {resume.why_in.map((line, i) => (
              <li
                key={i}
...
export function ResumeContent({ resume, variant, footer }: ResumeContentProps) {
  if (variant === "page") {
    return (
