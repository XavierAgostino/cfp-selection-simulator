...
import { REPO_DIR, RUNS_JSON_PATH } from "@/lib/paths";
...
async function loadCatalogFromRunsJson(): Promise<RunsPayload | null> {
  try {
    const raw = await readFile(RUNS_JSON_PATH, "utf-8");
    return JSON.parse(raw) as RunsPayload;
  } catch {
    return null;
  }
}
...
export async function loadRunCatalog(limit = 100): Promise<RunCatalogResponse> {
  ...
  const index = await loadCatalogFromRunsJson();
  if (index) {
    return {
      source: "runs_json",
      runs: index.runs,
