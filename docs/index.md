# CFP Selection Simulator Docs

Documentation funnel: **Quickstart → User Guide → Methodology → Research → Development**

---

## Start here

| Doc | Audience | Purpose |
|-----|----------|---------|
| [Quickstart](quickstart.md) | Everyone | Install and run in 5 minutes |
| [User Guide](user-guide.md) | Analysts, fans | Understand outputs and workflows |
| [CLI Reference](cli-reference.md) | Power users | Complete command reference |
| [Dashboard Guide](dashboard-guide.md) | Dashboard users | Streamlit UI walkthrough |
| [Output Files](output-files.md) | Data explorers | CSV/JSON column reference |
| [Configuration](configuration.md) | Researchers | YAML configs and reproducibility |

---

## Research and methodology

| Doc | Purpose |
|-----|---------|
| [Research index](research/index.md) | Methodology overview |
| [CFP Format History](research/cfp-format-history.md) | 4-team, 2024, 2025+ rules |
| [Model Methodology](research/model-methodology.md) | Composite pipeline |
| [Metric Definitions](research/metric-definitions.md) | SOR, SOS, résumé, predictive |
| [Historical Validation](research/historical-validation.md) | Backtest methodology |
| [Sensitivity Analysis](research/sensitivity-analysis.md) | Selection stability |
| [Data Sources](research/data-sources.md) | CFBD, caching, assets |
| [Limitations & Ethics](research/limitations-and-ethics.md) | Scope and responsible use |
| [Case studies](research/case-studies/) | Notable selection debates |

---

## Contributors

| Doc | Purpose |
|-----|---------|
| [Project Structure](project-structure.md) | Codebase layout |
| [Development Guide](development.md) | Setup, test, release |
| [Contributing](../CONTRIBUTING.md) | PR expectations |

---

## Legacy docs

Older standalone files are kept for reference but may be superseded:

- [METHODOLOGY.md](METHODOLOGY.md) → see [research/model-methodology.md](research/model-methodology.md)
- [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md) → see [project-structure.md](project-structure.md)
- [DOCKER.md](DOCKER.md) → optional Docker workflow
