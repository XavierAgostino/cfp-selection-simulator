...
export function DemoModeBannerClient() {
  ...
  return (
    <div
      role="status"
      className="relative border-b border-white/10 bg-[#0c0c0c] before:pointer-events-none before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-gradient-to-r before:from-transparent before:via-accent-gold/80 before:to-transparent"
    >
      <div className="mx-auto flex h-10 max-w-none items-center justify-center gap-2 px-11 sm:gap-3 sm:px-14">
        ...
        <span
          className="hidden h-3.5 w-px shrink-0 bg-white/15 sm:block"
          aria-hidden
        />
        ...
        >
          {PUBLIC_DEMO_BANNER_CTA}
          <LucideAppIcon icon={ArrowRight} size={14} strokeWidth={2} />
        </Link>
      </div>
      <Button
        type="button"
        ...
        size="icon-sm"
        onClick={dismiss}
        className="absolute top-1/2 right-2 size-7 -translate-y-1/2 text-muted-foreground hover:bg-white/8 hover:text-foreground sm:right-3"
        aria-label="Dismiss demo notice"
      >
        <LucideAppIcon icon={X} size={14} strokeWidth={2} />
      </Button>
    </div>
  );
}
