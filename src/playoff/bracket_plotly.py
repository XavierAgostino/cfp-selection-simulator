...
def create_interactive_bracket(
    ...
    for team in teams:
        ...
        pred = (
            f"#{int(team.get('predictive_rank', 99))}"
            if pd.notna(team.get("predictive_rank"))
            else "N/A"
        )
    ...
    for game in first_round:
        ...
        fig.add_shape(
            type="line", x0=mid, y0=(hy + ly) / 2, x1=qx - 0.4, y1=qy, line=dict(color="#888")
        )
    ...
    fig.add_annotation(
        x=fx,
        y=fy,
        text="<b>NATIONAL<br>CHAMPION</b>",
        font=dict(color="#DAA520", size=12),
        showarrow=False,
    )
    ...
    for x, y, text in labels:
        fig.add_annotation(
            x=x, y=y, text=f"<b>{text}</b>", showarrow=False, font=dict(size=11, color="#444")
        )
